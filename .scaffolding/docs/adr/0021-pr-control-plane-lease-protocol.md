...
**這是本層的選擇，不是契約的限制。** 向骨架層確認過：載體的假設只有「誰出示 capability，誰就持有 lease」，沒有長駐行程的預期，落地存完全允許。記下這一點是為了讓之後想改的人知道那扇門沒有鎖。附帶好處是該層若日後加入 capability 輪替（目前沒有機制也沒有規劃，要加會先通知），本層不需要改任何東西——沒有落地的東西就沒有輪替問題。
