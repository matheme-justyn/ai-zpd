...
- `[project].type`, `[project].features` and `[project].quality` moved to `[modules]`, with `type` renamed to `domain`. All six inputs to documentation-module loading now live in one section; `[project]` holds project metadata only. No compatibility period for the old keys — `config.toml` is generated by `configure-project-type.sh` rather than hand-written, and a silent fallback would make a misplaced key look valid (ADR 0017).
- `configure-project-type.sh` emits the new shape and no longer writes `mode = "project"`, a key removed in ADR 0016.
- README version badges corrected from `4.0.0` to `4.0.2`, and `ci.yml` now fails when a badge does not match `.scaffolding/VERSION`.

### Removed

- `generate-readme.sh`. AGENTS.md instructed agents to always use it and never edit README.md directly, but its declared source `.scaffolding/i18n/locales/` does not exist, the `toml_file` variable it declared was never read, and the README content was hardcoded inline from before the rename — running it as instructed would have overwritten the current READMEs with pre-split text titled "My Vibe Scaffolding" (ADR 0018).
- `.scaffolding/README.md` and `.scaffolding/README.zh-TW.md`. Copies made by that generator, last refreshed at 3.2.0, read by nothing.
...
- ADR 0014: records the ai-scheme/ai-zpd/ai-skill-web split, naming rationale, Dropbox mmap incident.
